...
states = ['seek', Timeout(name='tracking', timeout=5, on_timeout='timeout'), Timeout(name='tooMuchLook', timeout=5, on_timeout='timeout_seek'),Timeout(name='delayS', timeout=delaytime, on_timeout='delayS'),Timeout(name='delayT', timeout=delaytime, on_timeout='delayT')]
#遷移の定義
# trigger：遷移の引き金になるイベント、source：トリガーイベントを受ける状態、dest：トリガーイベントを受けた後の状態
# before：遷移前に実施されるコールバック、after：遷移後に実施されるコールバック
transitions = [
    { 'trigger': 'look', 'source': 'seek', 'dest': 'delayS'},
    { 'trigger': 'donotlook', 'source': 'tracking', 'dest': 'delayT'},
    { 'trigger': 'too_much_look', 'source': 'tracking', 'dest': 'tooMuchLook'},
    { 'trigger': 'back', 'source': 'tooMuchLook', 'dest': 'seek'},
    { 'trigger': 'delay_end_t2s', 'source': 'delayT', 'dest': 'seek'},
    { 'trigger': 'delay_end_s2t', 'source': 'delayS', 'dest': 'tracking'}
]
    ...
    while cap.isOpened():#0.04
        ...
        if ret:
            ...
            if len(face_rects) > 0:
                ...
                data[1] = lump.HeadAngle("vertical",cam_matrix[1,1],shape[28,1],height,0.5)   #0.00003

                if euler_angle[1,0]<18 and euler_angle[1,0]>-18 and lump.state != 'tooMuchLook' and lump.state !='delayS' or lump.state == 'delayT':
                    if lump.state =='seek':
                        lump.look()
                    else:
                ...
                elif euler_angle[1,0]>26 or euler_angle[1,0]<-26 or lump.state == 'tooMuchLook' or lump.state == 'delayS' and lump.state != 'delayT':
                ...
                if lump.state=='tracking'and lump.state != 'tooMuchLook' and lump.state !='delayS' or lump.state == 'delayT':
                    client1.send(str(data[0]).encode()) #データを送信します #
                    client2.send(str(data[1]).encode())
                elif lump.state=='seek'or lump.state == 'tooMuchLook' or lump.state == 'delayS' and lump.state != 'delayT':
