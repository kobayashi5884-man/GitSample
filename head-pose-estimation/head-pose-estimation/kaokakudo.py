...
states = ['seek', Timeout(name='tracking', timeout=5, on_timeout='timeout'), Timeout(name='tooMuchLook', timeout=5, on_timeout='timeout_seek'),Timeout(name='delay',timeout=delayTime,on_timeout='delay')]
#遷移の定義
# trigger：遷移の引き金になるイベント、source：トリガーイベントを受ける状態、dest：トリガーイベントを受けた後の状態
# before：遷移前に実施されるコールバック、after：遷移後に実施されるコールバック
transitions = [
    { 'trigger': 'look', 'source': 'seek', 'dest': 'delay'},
    ...
    { 'trigger': 'delay_end', 'source': 'delay', 'dest': 'tracking'}
]
    ...
    while cap.isOpened():#0.04
        ...
        if ret:
            ...
            if len(face_rects) > 0:
                ...
                if euler_angle[1,0]<18 and euler_angle[1,0]>-18 and lump.state != 'tooMuchLook' and lump.state!='delay':
                    if lump.state =='seek':
                        lump.look()
                    data_x = lump.HeadAngle("horizontal",cam_matrix[0,0],shape[28,0],width,0.5)  #
                    data_y = lump.HeadAngle("vertical",cam_matrix[1,1],shape[28,1],height,0.5)   #0.00003
                ...
                elif euler_angle[1,0]>26 or euler_angle[1,0]<-26 or lump.state == 'tooMuchLook' or lump.state =='delay':
                    ...
                    if lump.state == 'tracking':
                        ...
                        print('Send_y : %s' % str(data_y))                  #
                        client1.send(str(data_x).encode()) #データを送信します #
                        client2.send(str(data_y).encode())
                        print(euler_angle[1,0])
                        print("間で顔追跡＊＊＊")
