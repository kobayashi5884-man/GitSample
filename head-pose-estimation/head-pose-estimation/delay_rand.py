...
class Matter(object):
    ...
    def modeS2T(self):
        print('****** %s *****' % str(nowTime[0]))
        nowTime[0]=time.time()
        nowTime[1]=random.uniform(2, 5)
    ...
    while cap.isOpened():#0.04
        ...
        if ret:
            ...
            if len(face_rects) > 0:
                ...
                timeover = time.time()-nowTime[0]
                print(timeover)
                if lump.state == 'tracking':
                    if timeover > nowTime[1]:
                        lump.donotlook()
                    ...
                    if timeover > nowTime[1]:
                        lump.look()
                    print("do not look:%s"%str(nowTime))
                ...
                if lump.state=='tracking'and lump.state !='delayS' or lump.state == 'delayT':
                    client1.send(str(data[0]).encode()) #データを送信します #
                    client2.send(str(data[1]).encode())
                elif lump.state=='seek'or lump.state == 'delayS' and lump.state != 'delayT':
